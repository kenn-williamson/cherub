...
/// Type alias so callers (store constructors, test fixtures) can use `Pool`
/// without importing `sqlx::PgPool` directly.
...
/// exposed at the single point where sqlx requires a plain `&str`.
/// The binary is self-contained: migrations are embedded at compile time via
/// `sqlx::migrate!`. No migration files need to be deployed.
pub async fn connect(database_url: SecretString) -> Result<Pool, CherubError> {
    ...
    let pool = PgPoolOptions::new()
        ...
        .await
        .map_err(|e| CherubError::Storage(format!("failed to connect: {e}")))?;

    // Run migrations on startup. sqlx::migrate! is idempotent — already-applied
